...
void EventManager::firesendToDbRundownEvent(const SendDbRundownEvent& event)
{
    qDebug() << "*******Emit firesendToDbRundownEvent\n";

    emit SendDbRundown(event);//    emit sendToDatabase();
      //SendDbRundown();
}
