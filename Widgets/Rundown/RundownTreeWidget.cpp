...
void RundownTreeWidget::sendRundown(const SendDbRundownEvent& event)
{
    qDebug()<< "Into  SEnd DBRUNDOWN==========";
}

void RundownTreeWidget::SendDbRundown(const SendDbRundownEvent& event)
{
    qDebug() << "************ Into SendDbRundown************\n";
}
...
bool RundownTreeWidget::createConnection(QByteArray data)
{
   bool dbopen = false, db2open = false;

   QSqlDatabase db = QSqlDatabase::addDatabase("QMYSQL","mysql-connection");
   QString dbprimary = DatabaseManager::getInstance().GetServerIp(0);
   QString primarydbpass, primarydbuser;

   primarydbuser = DatabaseManager::getInstance().getDbUserName(dbprimary);
   primarydbpass = DatabaseManager::getInstance().getDbUserPass(dbprimary);

   qDebug() << "primaryuser, pass" << primarydbpass << primarydbuser;
   db.setHostName(dbprimary);
   db.setDatabaseName("casparcg");
   db.setUserName(primarydbuser);
   db.setPassword(primarydbpass);

   //------------------Sent Server2------------------------------
   QSqlDatabase db2 = QSqlDatabase::addDatabase("QMYSQL","mysql-connection2");
   QString dbshadow = DatabaseManager::getInstance().GetServerIp(1);

   QString shadowdbuser, shadowdbpass;

   shadowdbuser = DatabaseManager::getInstance().getDbUserName(dbshadow);
   shadowdbpass = DatabaseManager::getInstance().getDbUserPass(dbshadow);

   if (dbshadow.size() > 0) {
       db2.setHostName(dbshadow);
       db2.setDatabaseName("casparcg");
       db2.setUserName(shadowdbuser);
       db2.setPassword(shadowdbpass);

       qDebug() << db2.open();
       if (!db2.open()) {
           db2open = false;
           qDebug() << "Database2 error occurred";
       } else
           db2open = true;
   } else {
       qDebug() << "DB Shadow Not Available";
       db2open = false;
   }
   ...
   QDomElement items = doc.documentElement();
   QDomElement item = items.firstChildElement("item");
   QSqlDatabase::database().transaction();

   while (!item.isNull()) {
       QDomElement nodeType = item.firstChildElement();
       QString columnName;
       QString values;

       QString columnName1="INSERT INTO playlist(";
       QString values1="VALUES (";
       QString sqlQuery;

       while (!nodeType.isNull()) {
           QString nodeValueType = nodeType.firstChild().toText().data();
           if(nodeType.tagName()=="loop") {
               nodeType = nodeType.nextSibling().toElement();
               continue;
           }
           //qDebug() << nodeType.tagName() << nodeValueType;
           columnName.append(nodeType.tagName());
           values.append("'"+nodeValueType+"'");
       ...
       columnName.append("state");
       values.append("\'0\'");      // state indicates this video is not yet played
       columnName.append(",");
       values.append(",");
       columnName.append("pushtime");
       values.append("now()");
       columnName=columnName1.append(columnName).append(")");
       values=values1.append(values).append(")");

       sqlQuery=columnName.append(values);
...
void RundownTreeWidget::InsertIntoDb(void)
{
    QByteArray data;
    ...
    for (int i = 0; i < this->treeWidgetRundown->invisibleRootItem()->childCount(); i++) {
        qDebug() << "TreeWidgetItem " << this->treeWidgetRundown->invisibleRootItem()->text(i) << "\n";
    ...
    if (!this->createConnection(data))
        qDebug() << "Failed To Insert Into DB\n";
    else
        qDebug() << "Inserted into databse successful\n";
}
...
void RundownTreeWidget::saveRundown(bool saveAs)
{
    ...
    if (!path.isEmpty())
    {
        ...
        if (file.open(QFile::WriteOnly))
        {
            ...
            for (int i = 0; i < this->treeWidgetRundown->invisibleRootItem()->childCount(); i++) {
                qDebug() << i << " = writer contains :" << writer;
                this->treeWidgetRundown->writeProperties(this->treeWidgetRundown->invisibleRootItem()->child(i), writer);
            }
