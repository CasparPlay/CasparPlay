    ...
    : QDialog(parent)
{
    ...
    QObject::connect(this->toolButtonAddRazunaKey, SIGNAL(clicked()), this, SLOT(showRazunaDialog()));
    QObject::connect(this->toolButtonRemoveRazunaKey, SIGNAL(clicked()), this, SLOT(DeleteRazunaApiKey()));
...
void SettingsDialog::loadRazunaInfo(void)
{
    this->treeWidgetRazuna->clear();
    QList<RazunaModel> models = DatabaseManager::getInstance().getRazuna();

    foreach(RazunaModel model, models)
    {
        QTreeWidgetItem *treeItem = new QTreeWidgetItem(this->treeWidgetRazuna);
        treeItem->setText(0, QString("%1").arg(model.getId()));
        treeItem->setText(1, model.getName());
        treeItem->setText(2, model.getApiKey());
    }
}

void SettingsDialog::showRazunaDialog(void)
{
    qDebug() << "Show Razuna Dialog...";

    RazunaDialog *dialog = new RazunaDialog(this);
    if (dialog->exec() == QDialog::Accepted)
    {
        DatabaseManager::getInstance().insertRazuna(RazunaModel(0, dialog->getname(), dialog->getApiKey()));
    }
}

void SettingsDialog::DeleteRazunaApiKey()
{
    if (this->treeWidgetRazuna->selectedItems().count() == 0)
        return;

    DatabaseManager::getInstance().DeleteRazuna(this->treeWidgetRazuna->currentItem()->text(0).toInt());
    delete this->treeWidgetRazuna->currentItem();
}
        ...
        if (dialog->exec() == QDialog::Accepted)
        {
            ...
            foreach (DeviceModel model, models)
            {
                DatabaseManager::getInstance().insertDevice(DeviceModel(0, model.getName(), model.getAddress(),
                                                                        ...
                                                                        model.getDbUserName(),model.getDbUserPass()));
            }
...
void SettingsDialog::showAddDeviceDialog()
{
    ...
    if (dialog->exec() == QDialog::Accepted)
    {
        DatabaseManager::getInstance().insertDevice(DeviceModel(0, dialog->getName(), dialog->getAddress(),
                                                                ...
                                                                dialog->getDbUserName(), dialog->getDbUserPass()));
...
void SettingsDialog::deviceItemDoubleClicked(QTreeWidgetItem* current, int index)
{
    ...
    if (dialog->exec() == QDialog::Accepted)
    {
        DatabaseManager::getInstance().updateDevice(DeviceModel(model.getId(), dialog->getName(), dialog->getAddress(),
                                                                ...
                                                                dialog->getPreviewChannel(),dialog->getDbUserName(),dialog->getDbUserPass()));
