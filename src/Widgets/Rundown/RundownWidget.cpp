    ...
    : QWidget(parent)
{
    ...
    QObject::connect(&EventManager::getInstance(), SIGNAL(sendRundown(const SendDbRundownEvent&)), this, SLOT(sendRundown(const SendDbRundownEvent&)));
}
...
void RundownWidget::InsertIntoDB()
{
    qDebug() << "putting into database\n";
//    dynamic_cast<RundownTreeWidget*>(this->tabWidgetRundown->widget(i))->gpiBindingChanged(gpiPort, binding);
    dynamic_cast<RundownTreeWidget*>(this->tabWidgetRundown->currentWidget())->InsertIntoDb();
}

void RundownWidget::SendDbRundown(const SendDbRundownEvent& event)
{
    qDebug() << "************ Into RundownWidget::SendDbRundown************\n";
}

// rakib inclusion
void RundownWidget::sendRundown(const SendDbRundownEvent& event)
{
    qDebug()<< "INTO RundownWidget::sendRundown()******** \n";
    //EventManager::getInstance().firesendToDbRundownEvent()
    //dynamic_cast<RundownTreeWidget*>(this->tabWidgetRundown->currentWidget())->sendRundown(event.SendDbRundownEvent);
}
