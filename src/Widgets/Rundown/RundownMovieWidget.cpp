      ...
      clearVideolayerControlSubscription(NULL), clearChannelControlSubscription(NULL), reverseOscTime(false), sendAutoPlay(false), hasSentAutoPlay(false), useFreezeOnLoad(false)
{
    ...
    this->checkBoxCommercial->setVisible(true);
    //this->checkBoxCommercial->isEnabled(true);
    this->timeEdit->setDisabled(true);

    this->executeStartTimer.setSingleShot(true);
    this->executeStopTimer.setSingleShot(true);
    //QObject::connect(&this->checkBoxCommercial, SIGNAL(clicked()), this, SLOT(commercialisChanged()));
    ...
    QObject::connect(this->TimerEnablecheckBox,SIGNAL(clicked()), this, SLOT(EnableTimer()));
...
void RundownMovieWidget::EnableTimer()
{
    if (this->TimerEnablecheckBox->isChecked()) {
        this->timeEdit->setEnabled(true);
        qDebug() <<"Selected Time: " << this->timeEdit->time().toString("hh:mm:ss");
    }
    if (!this->TimerEnablecheckBox->isChecked())
...
void RundownMovieWidget::readProperties(boost::property_tree::wptree& pt)
{
    ...
    if (QString::fromStdWString(pt.get(L"commercial",L"")) == QString('0'))
        this->checkBoxCommercial->setChecked(false);
    else
        this->checkBoxCommercial->setChecked(true);
}
...
void RundownMovieWidget::writeProperties(QXmlStreamWriter* writer)
{
    ...
    if (this->checkBoxCommercial->isChecked())
        writer->writeTextElement("commercial", QString('1'));
    else
        writer->writeTextElement("commercial", QString('0'));
    if (this->TimerEnablecheckBox->isEnabled())
        writer->writeTextElement("starttime", QString(this->timeEdit->time().toString("hh:mm:ss")));
    else
        writer->writeTextElement("starttime", QString('0'));
}
