...
class CORE_EXPORT DatabaseManager
{
    public:
        ...
        QString GetServerIp(int shadow);

        QList<RazunaModel> getRazuna();
        QString GetRazunaApiKey();
        QString getDbUserName(QString server);
        QString getDbUserPass(QString server);
        void insertRazuna(const RazunaModel& model);
        void DeleteRazuna(int id);

        QString GetScrollerText(void);
        void ScrollerTextDel(void);
        void ScrollerTextInsert(QString text);
        void ScrollerTextUpdate(QString text);
